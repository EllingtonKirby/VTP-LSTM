...
trSet = ngsimDataset(
    grid_file='./ngsim_us101_D.csv', 
    tracks_file='./ngsim_us101_T.npy',
    t_f=5,
    t_h=15,
    d_s=1
)
# valSet = ngsimDataset('./ngsim_us101_D.csv')
...
for epoch_num in range(trainEpochs):
    ...
    for i, data in enumerate(trDataloader):
        ...
        l, acc = maskedMSE(fut_pred, fut, op_mask)#maskedNLL(fut_pred, fut, op_mask)
        ...
        avg_lat_acc += acc[:,:,0].sum()
        avg_lon_acc += acc[:,:,1].sum()

        if i%100 == 99:
            eta = avg_tr_time/100*(len(trSet)/batch_size-i) # average time/batch * rest batches
            # len(trset) total length; i * batch_size / len(trSet)
            print("Epoch no:",epoch_num+1,"| Epoch progress(%):",format(i/(len(trSet)/batch_size)*100,'0.2f'), "| Avg train loss:",format(avg_tr_loss/100,'0.4f'),"| Acc:",format(avg_lat_acc),format(avg_lon_acc), "| Validation loss prev epoch",format(prev_val_loss,'0.4f'), "| ETA(s):",int(eta))
